...
bool mycmp(tuple<ll,ll,ll>a,tuple<ll,ll,ll>b)
{
    if(get<0>(a)==get<0>(b))
    {
...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        //   FOR(i,0,n)
        // {
        //     cout<<get<0>(inp[i])<<" "<<get<1>(inp[i])<<endl;
        // }  
        ...
        FOR(i,0,n)
        {
            maxi[i]=max(maxi_c,get<1>(inp[i]));
            mini[n-1-i]=min(mini_c,get<0>(inp[n-1-i]));
            mini_c=mini[n-1-i];
            maxi_c=maxi[i];
        }
        // cout<<" ** "<<endl;
