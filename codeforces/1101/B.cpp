...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        cin>>str;
        bool flg1=false;
        ll open_ind=-1,open_col=-1,closing_col=-1,closing_ind=-1;
        vector<ll> cls;
        FOR(i,0,str.size())
        {
            if(open_ind==-1 && str[i]=='[')
            ...
            if(str[i]==']')
            {
                closing_ind=i;
                if(cls.size()>0)
                closing_col=cls[cls.size()-1];
            }
        }  
        if(open_ind==-1 || open_col==-1 || open_col==closing_col||closing_ind==-1 ||open_ind>closing_ind || closing_ind<open_col)
        {
