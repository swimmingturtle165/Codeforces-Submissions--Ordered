...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        // FOR(i,0,m) cout<<edges[i][0]<<" "<<edges[i][1]<<" "<<edges[i][2]<<endl;
        ...
        while(done!=n-1)
        {
            FOR(i,0,m)
            {
                if(find(parent,edges[i][0],edges[i][1])==false)
                {
                    done++;
                 
                    weighted_union(parent,sz,edges[i][1],edges[i][0]);
                    ans+=max(edges[i][2]-k,0ll);
