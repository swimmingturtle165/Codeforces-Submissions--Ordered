...
vector<ll> seive(5000001,-1);
vector<ll>prime_factors(5000001,0);
void solve ()
{
    seive[1]=1;
    FOR(i,2,250001)
    ...
    {
        if(seive[i]!=-1)
        prime_factors[i]=prime_factors[i/seive[i]]+1;
        else
        {
            prime_factors[i]=1;
        }
...
signed main(int argc, char** argv)
{
    ...
    solve();
        FOR(j,1,5000001)
        {
            prime_factors[j]+=prime_factors[j-1];
        }
    while(t--)
    {
        ll ans=0;
