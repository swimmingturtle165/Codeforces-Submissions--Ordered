...
signed main(int argc, char** argv)
{
    ...
    vector<ll> inp(n),neg_ans(n),pos_ans(n);
    FOR(i,0,n)
    {
        cin>>inp[i];
    }
    FOR(i,0,n)
    {
        ...
        else
        {
            if(inp[i]<0)
            {
                neg_ans[i]=pos_ans[i-1]+1;
                pos_ans[i]=neg_ans[i-1];
            }
            else
            {
                pos_ans[i]=pos_ans[i-1]+1;
                neg_ans[i]=neg_ans[i-1];
            }
    ...
    ll mini_ans=0,maxi_ans=0;
    FOR(i,0,n)
    {
        mini_ans+=neg_ans[i];
        maxi_ans+=pos_ans[i];
    }
    cout<<mini_ans<<" "<<maxi_ans<<endl;
    return 0;
}
