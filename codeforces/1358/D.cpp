...
signed main(int argc, char** argv)
{
    ...
    while(t--)
    {
        ...
        n*=2;
        vector<ll> sum_len(1,0),sum_val(1,0);
        FOR(i,0,n)
        {
            sum_len.push_back(sum_len.back()+inp[i]);
            sum_val.push_back(sum_val.back()+(inp[i]*(inp[i]+1))/2);
        }
        ll ans=0;
        FOR(i,1,n+1)
        {
            if(sum_len[i]>=x)
            {
                int idx=upper_bound(sum_len.begin(),sum_len.end(),sum_len[i]-x)-sum_len.begin();
                ll tmp=sum_val[i]-sum_val[idx];
                ll days=sum_len[i]-sum_len[idx];
                ll v=(x-days);
                ll v1=inp[idx-1];
                tmp+=(v1*(v1+1))/2;
                tmp-=((v1-v)*(v1-v+1))/2;
